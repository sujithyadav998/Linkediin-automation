...
#     driver = webdriver.Chrome(
#         options=options, service=service)  # For mac use -> /usr/local/bin/chromedriver
...
def configurations():
    ...
    driver = webdriver.Chrome(service=service, options=options)  # For mac use -> /usr/local/bin/chromedriver
...
def main():
    ...
    for keyword in list(constants.commaseparated.split(';')):
        ...
        for page in range(1, int(constants.upto_page) + 1):
            ...
            webpage.visit(link)
            time.sleep(10)
            list_of_cards = webpage.grab_elements_with_css_selector(
                "li[class='reusable-search__result-container']")
            ...
            for i in range(1, len(list_of_cards) + 1):
                ...
                try:
                    ...
                    if button_on_card and button_text == 'Connect':
                        ...
                        #connect_xpath = '//*[@id="ember123"]'

                        
                        # webpage.click_with_xpath_selector(connect_xpath)
                        # time.sleep(0)

                        x = "/html/body/div[6]/div[3]/div/div/div[2]/div/div/main/section[1]/div[2]/div[3]/div/button"
                        connect_button = driver.find_elements(By.XPATH, x)
                        #connect_button = driver.find_element(By.XPATH, "//button[contains(@class, 'pv-s-profile-actions')]")
